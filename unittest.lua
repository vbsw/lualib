...
function testCL()
	...
	assertEq(cmdLine.n, 2)
	local delimiter = cl.newDelimiter("=", "")
	asserType(delimiter, "table")
	assertMemberType(delimiter, {"hasEmpty", "hasSpace", "n"}, {"boolean", "boolean", "number"})
	assertEq(#delimiter, 1)
	assertEq(delimiter[1], "=")
	assertEq(delimiter.n, 1)
	assertEq(delimiter.hasEmpty, true)
	assertEq(delimiter.hasSpace, false)
	assertEq(delimiter:available(), true)
	argument = cmdLine:search(delimiter, "-s", "--start")
	assertEq(argument.n, 1)
	...
	delimiter = cl.newDelimiter(" ", "=")
	assertEq(#delimiter, 1)
	assertEq(delimiter[1], "=")
	assertEq(delimiter.n, 1)
	assertEq(delimiter.hasEmpty, false)
	assertEq(delimiter.hasSpace, true)
	assertEq(delimiter:available(), true)
	argument = cmdLine:search(delimiter, "-s", "--start")
	assertEq(argument.n, 1)
	assertEq(argument.values[1], "123")
